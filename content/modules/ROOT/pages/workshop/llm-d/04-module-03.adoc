...
Confirm all 4 pods show `READY` as `2/2` and `STATUS` as `Running` before proceeding.
...
image::grafana-model-pulldown.png[Grafana model selector pulldown showing llama-vllm-scaled,link=self,window=blank]
...
image::benchmark-dashboard-scaled.png[Screenshot of the Grafana dashboard showing scaled vLLM benchmark metrics,link=self,window=blank]
